"""
Pro-version pipeline (no CLIP):
- Reuse functions from src.utils.* (extractor, compare_text, compare_table, compare_image, report)
- Semantic text comparison using sentence-transformers 'all-MiniLM-L6-v2'
- Summarization via FLAN‑T5 (if available), with fallback
- Caching of embeddings to disk (.cache_embeddings/)
- Keep memory usage low on CPU
"""
...
def compare_pdfs_pro(pdf_a: str, pdf_b: str, out_html: str | None = None) -> Dict[str, Any]:
    ...
    # Image compare: phash-based similarity only
